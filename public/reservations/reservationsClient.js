function creerBouton(id, action) {
    ...
    }else if (action === "affBillets") {
        bouton.textContent = "Voir les billets";
    }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    for (let i=0; i<data.length; i++) {
        ...
        if (row.statut === `payé`) {
            tdAction.append(creerBouton(row.id, `affBillets`), creerBouton(row.id, `annuler`));
        }else if (row.statut === "en attente") {
            tdAction.append(creerBouton(row.id, `annuler`));
        }
